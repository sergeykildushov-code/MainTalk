...
export function createCallboxWindow(params: CallboxParams) {
	...
	// читаем размеры из config.json, если есть
	const callboxWidth = BUILD_CONFIG.callboxWidth ?? 600
	const callboxHeight = BUILD_CONFIG.callboxHeight ?? 300
	...
	const window = new BrowserWindow({
		...
		// фон из config.json (brandColor), fallback — синий
		backgroundColor: BUILD_CONFIG.brandColor ?? '#00679E',
		frame: false,
	...
	// показываем активно (поверх всех окон)

 ...
 * Всегда возвращает значение applicationName из build/config.json.
 * Любые изменения надписи производятся только в build/config.json.
 */
export function buildTitle(_maybeTitle?: string): string {
    return BUILD_CONFIG.applicationName ?? 'Nextcloud Talk'
}
